...
let envs = \(x : List KV) -> 
  ...
  Some (P.List.map KV k8s.EnvVar.Type f x)

let defaultNotes = 
  P.List.concatMap C KV (\(service : C) ->   
    let serviceName = service.name in 
    [     
...
let fileMap = \(readOnly : Bool) -> \(pathType : PathType) -> \(pathShareType : PathShareType) -> \(hostPath : Text) -> \(containerPath: Text) ->
  ...
  in 
    { 
      notes = merge { Shared = shared, Private = private } pathShareType
    , volume = 
...
let mount = \(vs : List V) -> Some (P.List.map V k8s.VolumeMount.Type (\(v : V) -> v.volumeMount) vs)
let mountVolumes = \(vs : List (List V)) -> Some (P.List.map V k8s.Volume.Type (\(v : V) -> v.volume) (P.List.concat V vs))
let mountNotes = P.List.map V KV (\(v : V) -> v.notes)
...
let baseDeployment = 
    \(podName : Text) -> \(services: List C) -> \(volumes: List (List V)) ->
    ...
    , metadata = k8s.ObjectMeta::{
      ...
      , annotations = Some (P.List.concat KV
                  [ 
                      mountNotes (P.List.concat V volumes)
                    , defaultNotes services
                  ]
                )
    }
...
in  baseDeployment "nextcloud-pod" [ postgres, redis, nextcloud ] [ postgresVolumes, nextcloudVolumes]
